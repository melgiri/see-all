...
def see(i):
    ...
    s.settimeout(5)
    target = socket.gethostbyname(ip[i])
    conn = s.connect_ex((target, port[i]))
    if not conn:
        print(f"The device with name {name[i]} which has ip {target} and port {port[i]} is Active right now")
    else:
        print(f"The device with name {name[i]} which has ip {target} and port {port[i]} is InActive right now")
        if cd['discord'] == "True":
            data["content"] = f"<@{user_id}>\nThe device with name `{name[i]}` which has ip `{target}` and port `{port[i]}` is `InActive` right now"
